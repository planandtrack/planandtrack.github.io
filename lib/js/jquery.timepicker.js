...
(function (factory) {
	...
	var methods =
	{
		...
		{
			var self = this;
			var prettyTime = _int2time(_time2int(value));
	...
	function _render(self)
	{
		...
		if (end === _ONE_DAY-1 && moment().localeData().longDateFormat('LT').indexOf('HH') !== -1) {
		...
		for (var i=start; i <= end; i += settings.step*60) {
			var timeInt = i;
			var timeString = _int2time(timeInt);
	...
	function _roundTime(time, settings)
	{
		...
		if (time === null) {
			...
			return _int2time(Math.round(time / step) * step);
		}
	}
	...
	function _formatValue()
	{
		...
		var prettyTime = _int2time(seconds);
	...
	*/
	...
	function _selectValue(self)
	{
		...
		if (timeValue !== null) {
			if (typeof timeValue == 'string') {
				...
				var timeString = _int2time(timeValue);
	...
	function _time2int(timeString)
	{
		...
		if (typeof(timeString) == 'object') {
			timeString = moment(timeString).format('LT');
		}
		
		var time = moment(timeString, 'LT');
